...
def concat_image(imgs):
    ...
    output = output.view(-1, 1, x, y, z)  # [batch_size, 1, 96, 96, 96]
    return output
...
def concat_image(imgs):
    ...
    for img in imgs:
        ...
        if not isinstance(img, dict):
            raise TypeError(f"Expected a dictionary, but got {type(img)}")
        if 'image' not in img:
            raise KeyError("Each dictionary must have an 'image' key")
        if not isinstance(img['image'], torch.Tensor):
            raise TypeError("The 'image' key must correspond to a torch.Tensor")

        output.append(img['image'])

    output = torch.cat(output, dim=1)

    try:
        bs, sw_s, x, y, z = output.size()
        output = output.view(-1, 4, x, y, z)
    except RuntimeError as e:
        raise RuntimeError(f"Error in reshaping the tensor: {str(e)}")
